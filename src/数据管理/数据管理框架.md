## 数据管理框架（Data Management Frameworks）

- 战略一致性模型（Strategic Alignment Model）和阿姆斯特丹信息模型（ Amsterdam Information Model）展示了高阶关系如何影响组织管理数据
- DAMA DMBOK框架（The DAMA Wheel, Hexagon, and Context Diagram）描述了DAMA定义的数据管理知识领域，并解释了其在DMBOK中的可视化表示方式（visual representation）
- 最后两个模型以DAMA轮为起点重新排列（rearrange）各部分，以便更好地描述和理解各部分之间的关系
