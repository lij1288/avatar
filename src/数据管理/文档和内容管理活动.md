...
- 文档/记录管理组件（Component）及对应的审计指标

  - 库存（Inventory）
    - 库存中的每个位置都是唯一标识的
  - 存储（Storage）
    - 用于存储物理文档/记录的区域有足够的空间来容纳增长
  - 可靠性和准确性（Reliability and Accuracy）
    - 进行抽查以确认文档/记录充分反映了创建或接收的内容
  - 分类和索引方案（Classification and Indexing Schemes）
    - 很好地描述了元数据和文档文件规划
  - 访问和检索（Access and Retrieval）
    - 最终用户可以轻松地查找和检索关键信息
  - 保管流程（Retention Processes）

  ![](assets/文档和内容管理活动/审计指标示例.jpg)

## 发布和传递内容（Publish and Deliver Content）

### 提供访问、搜索和检索（Provide Access, Search, and Retrieval）

- 一旦内容通过元数据或关键字标记描述并在何时的信息内容体系中进行分类，则可以被检索和使用。
- 维护用户配置文件的门户技术可以帮助找到非结构化数据。搜索引擎可以根据关键字返回内容。一些组织的专业人员通过内部搜索工具检索信息。

### 通过可接受的渠道传递（Deliver Through Acceptable Channels）

- 随着内容用户想在其选择的设备上消费或使用内容，交付期望发生了变化。如需要再Word中创建内容并移至HTML，或为某个平台、某个屏幕分辨率或屏幕上给定尺寸提供内容。
- 有可能需要将任何更改的内容同步回原格式。